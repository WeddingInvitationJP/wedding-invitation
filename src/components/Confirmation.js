...
const ModalContent = styled(Box)({
  overflowY: 'auto',
  flexGrow: 1,
  marginTop: '40px', // Space for the close button and icon
});
    ...
    <>
      <CardComponent>
        ...
        <Typography variant="body1" sx={{ marginTop: '20px', color: '#855D41' }}>
          Estamos agradecidos con tu presencia y nos encantará contar con vosotros en ese día tan especial.
        </Typography>
      ...
      <Modal open={open} onClose={handleClose}>
        <ModalContainer>
          ...
          <ModalContent ref={contentRef}>
            ...
            <Typography variant="h6" sx={{ marginBottom: '20px', color: '#855D41' }}>
              Confirmar asistencia con ...
            </Typography>
            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
              <IconButtonStyled onClick={() => handleSendMessage(`+34${phoneNumberFran}`)}> {/* Reemplaza con el número del novio */}
                <AccountCircleIcon sx={{ fontSize: '4rem' }} />
                <Typography variant="body2" sx={{ color: '#855D41', marginTop: '10px' }}>Fran</Typography>
              </IconButtonStyled>
              <IconButtonStyled onClick={() => handleSendMessage(`+34${phoneNumberLaura}`)}> {/* Reemplaza con el número de la novia */}
